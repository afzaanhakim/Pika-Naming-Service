...
error InvalidName(string name);
  ...
  function register (string calldata name) public payable { //a register function that takes name as parameter and assignes it to whoeevr calls this func on the chain (msg.sender) and adds the name to the domains mapping - calldata is the location of the where name argument should be stored. I am storing in calldata so it takes less gas.
   ...
   // Check if enough Matic was paid in the transaction
    ...
    string memory _name = string(abi.encodePacked(name, ".", maindomain)); //combining the name passed with the maindomain

    string memory finalSvg = string(abi.encodePacked(svgPartOne, _name, svgPartTwo)); //creating the SVG image for the NFT with the name

    uint256 newRecordId = _tokenIds.current();
    uint256 length = StringUtils.strlen(name);
      string memory strLen = Strings.toString(length);

      console.log("Registering %s.%s on the contract with tokenID %d", name, maindomain, newRecordId);
      ...
      bytes(
        string(
          abi.encodePacked(
            '{"name": "',
            _name,
            '", "description": "A domain on the PIKA name service", "image": "data:image/svg+xml;base64,',
    ...
    _safeMint(msg.sender, newRecordId); //mint nft
    _setTokenURI(newRecordId, finalTokenUri); //set nft data - the json with info
